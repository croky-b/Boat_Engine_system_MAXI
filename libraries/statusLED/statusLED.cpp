...
// servo setpos function ************************************************************
void statusLED::setpos(float pos)
{
  _pos = pos;
  _duty = (((_pos / 180.0)
...
void statusLED::sweep(float startsweep, float end)
{
  _startsweep = startsweep;
  _end = end;

  if (_startsweep < _end) {

    if ( _possweep <= startsweep) {
      _possweep	= _startsweep;
      _temps = millis();

      while (_possweep < _end)

      {
        if ((millis() - _temps) > 10) {
          _possweep++;
          _dutysweep = (((_possweep / 180.0)
                         * 2000) / 20000.0 * 65536.0) + 1634;

          ledcWrite(_channel, _dutysweep);
...
bool statusLED:: home()const
{
return (_home);

}

bool statusLED::is_moving()const
{ 
return (_move);
}
