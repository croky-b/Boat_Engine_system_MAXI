 ...
 */
SlowMotionServo::SlowMotionServo(byte pin, byte unit, byte timer,byte channel) :
  SlowMotionServo()
{
  mPin = pin;
  mUnit = unit;
...
void SlowMotionServo::goTo(float position)
{
  ...
  if (mTargetRelativeTime > mInitialRelativeTime) {
    mState = SERVO_UP;
    if (mPin != NOPIN) attach(mPin,mUnit,mTimer,mChannel);
  }
...
void SlowMotionServo::setupMin(uint16_t minPulse)
{
  if (isSettable()) {
    if (mState != SERVO_SETUP) {
      mSavedState = mState;
      if (! attached()) attach(mPin,mUnit,mTimer,mChannel);
    }
    mState = SERVO_SETUP;
    minPulse = constrainPulse(minPulse);
    if (minPulse > mMaxPulse) minPulse = mMaxPulse;
