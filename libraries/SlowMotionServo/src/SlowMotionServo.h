...
public:
  SlowMotionServo();
  SlowMotionServo(byte pin,byte unit, byte timer,byte channel);
  virtual ~SlowMotionServo();
  void setPin(const byte pin,const byte unit,const byte timer,const byte channel) { mPin = pin , mUnit= unit,mTimer=timer, mChannel=channel; }
