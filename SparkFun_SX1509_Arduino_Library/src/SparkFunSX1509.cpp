...
void SX1509::blinkStop(byte pin)
{
		
	setupBlink(pin, 0, 0, 255, 0, 0, 0);
}

void SX1509::On(byte pin)
{
		
	writePin(pin, HIGH);
}

bool SX1509::Off(byte pin)
{	
...
bool SX1509::neonOn(byte pin,byte sequence)
{ 
		
	if (index<35){
  if (millis()-millisNeon > startSeq[sequence][index])
    {
      millisNeon=millis();
      state=!state;
      writePin(pin, state);
...
bool SX1509::halogenOff(byte pin,byte fadeIncrement,byte fadeInterval )
{
	if (!LightOn){
  if (millis() - FadeMillis >= fadeInterval) {
    ...
    if (fadeValue >= maxPWM) {           //<-- Added if block
      // At max, limit and change direction
      fadeValue = maxPWM;
      LightOn =true;
      //fadeIncrement = - fadeIncrement;   //<-- Invert the value of "fadeIncrement"
    }
...
bool SX1509::halogenOn(byte pin,byte fadeIncrement,byte fadeInterval )
{ 
if (LightOn){
  if (millis() - FadeMillis >= fadeInterval) {
    FadeMillis=millis();
...
// Flash function ************************************************************
bool SX1509::flash(byte pin,unsigned long onDuration, unsigned long offDuration, unsigned long pauseDuration, int pulses, int delay) {
...
// State machine
    switch (_Flashstate) {
        case 0: //---- Step 0 (do nothing)
            _previousFlashMillis = millis();
            _Flashstate = 1;
            break;
            
        case 1: //---- Step 1 (Start delay for first pass)
            if (millis() - _previousFlashMillis >= _delay) {
                _Flashstate = 2;
            }
            break;
        ...
        case 3: //---- Step 3 (ON duration)
            if (millis() - _previousFlashMillis >= _onDuration) {
                _Flashstate = 4;
        ...
        case 5: //---- Step 5 (OFF duration)
            if (millis() - _previousFlashMillis >= _offDuration) {
                _Flashstate = 6;
            }
            break;
        ...
        case 7: //---- Step 7 (Pause duration)
            if (currentFlashMillis - _previousFlashMillis >= _pauseDuration) {
                _Flashstate = 2;
            }
            break;
